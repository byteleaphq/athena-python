...
class Document:
    ...
    def search_documents(self, brain_id: str, search_query: Optional[str] = None) -> operations.SearchDocumentsResponse:
        r"""Search documents
        Search for documents within a specific brain
        """
        hook_ctx = HookContext(operation_id='searchDocuments', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        url = utils.generate_url(base_url, '/document/{brain_id}/search', request)
        ...
        res = operations.SearchDocumentsResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
    ...
    def create_document_review(self, request: operations.CreateDocumentReviewRequestBody) -> operations.CreateDocumentReviewResponse:
        r"""Create document review
        Create a new document review
        """
        hook_ctx = HookContext(operation_id='createDocumentReview', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        base_url = utils.template_url(*self.sdk_configuration.get_server_details())
        
        url = base_url + '/document/review'
        ...
        req_content_type, data, form = utils.serialize_request_body(request, operations.CreateDocumentReviewRequestBody, "request", False, False, 'json')
        if req_content_type is not None and req_content_type not in ('multipart/form-data', 'multipart/mixed'):
            headers['content-type'] = req_content_type
        ...
        res = operations.CreateDocumentReviewResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
        
        if http_res.status_code == 200:
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[components.DocumentReview])
                res.document_review = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
        elif http_res.status_code >= 400 and http_res.status_code < 500 or http_res.status_code >= 500 and http_res.status_code < 600:
    ...
    def list_document_reviews(self, brain_id: Optional[str] = None) -> operations.ListDocumentReviewsResponse:
        r"""List document reviews
        Retrieve a list of document reviews
        """
        hook_ctx = HookContext(operation_id='listDocumentReviews', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        res = operations.ListDocumentReviewsResponse(http_meta=components.HTTPMetadata(request=req, response=http_res))
        
        if http_res.status_code == 200:
            # pylint: disable=no-else-return
            if utils.match_content_type(http_res.headers.get('Content-Type') or '', 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[List[components.DocumentReviewDetail]])
                res.document_review_details = out
            else:
                content_type = http_res.headers.get('Content-Type')
                raise errors.SDKError(f'unknown content-type received: {content_type}', http_res.status_code, http_res.text, http_res)
        elif http_res.status_code >= 400 and http_res.status_code < 500 or http_res.status_code >= 500 and http_res.status_code < 600:
